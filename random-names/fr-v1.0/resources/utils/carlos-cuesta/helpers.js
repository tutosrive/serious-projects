...
export default class Helpers {
  ...
  /**
   * Busca por la propiedad text, un elemento <select> y selecciona una opción
   * @param {Element} select La referencia al <select> en la cual se hará la búsqueda
   * @param {String} text La cadena de texto a buscar
   */
  static selectOptionByText(select, text) {
    // Buscar el índice de la opción cuyo texto coincida con el argumento recibido
    const index = [...select.options].findIndex(option => option.text.trim().toLowerCase() === text.trim().toLowerCase())
    // Seleccionar el elemento con el índice encontrado
